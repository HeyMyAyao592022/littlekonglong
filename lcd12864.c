...
xdata const uint8 img_nums[10][16] = {
...
void drawKonglong(KongLong *konglong)
{
	...
	// 判断是否在跨越左右屏幕
	if (konglong->x > 47 && konglong->x < 65)
	{
...
void playNums(uint8 x, uint8 y, uint8 num)
{
	...
	cmd_w12864(0x34); // 扩充指令
	cmd_w12864(0x36); // 绘图指令
	...
	cmd_w12864(0xb8 | (y + 1));
	for (i = 0; i < 8; i++)
		dat_w12864(img_nums[digit][i + 8]);

	digit = num % 10;
	cmd_w12864(0x40 | (x + 16));
	cmd_w12864(0xb8 | y);
	for (i = 0; i < 8; i++)
		dat_w12864(img_nums[digit][i]);
...
// 	cmd_w12864(0xb8 | (y + 2));
// 	for (i = 0; i < 32; i++)
// 		dat_w12864(addr[63 + i]);

// 	cmd_w12864(0x40 | x);
// 	cmd_w12864(0xb8 | (y + 3));
// 	for (i = 0; i < 32; i++)
// 		dat_w12864(addr[95 + i]);
// }
